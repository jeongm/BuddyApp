// app/(tabs)/diary.tsx
import { Ionicons } from '@expo/vector-icons';
import { format } from "date-fns";
import { ko } from "date-fns/locale";
import { Image } from "expo-image";
import { useFocusEffect, useRouter } from "expo-router";
import React, { useCallback, useState } from "react";
import { ActivityIndicator, Dimensions, FlatList, RefreshControl, Text as RNText, ScrollView, TextInput, TouchableOpacity, useColorScheme, View } from "react-native";
import { SafeAreaView } from "react-native-safe-area-context";

import { diaryApi } from "../../api/diaryApi";
import { AppText as Text } from '../../components/AppText';
import { useAuthStore } from "../../store/useAuthStore";
import { useSettingStore } from "../../store/useSettingStore";
import { ACCENT_HEX_COLORS, useThemeStore } from "../../store/useThemeStore";
import type { DiarySummary } from "../../types/diary";

const { width } = Dimensions.get('window');
const scale = (size: number) => Math.round((width / 430) * size);

type ViewModeType = "all" | "photo";
const PAGE_SIZE = 15;

export default function DiaryFeedScreen() {
    const router = useRouter();
    const { user } = useAuthStore();
    const { fontFamily } = useSettingStore();
    const customFontFamily = fontFamily === 'System' ? undefined : fontFamily;

    const { accent } = useThemeStore();
    const accentHex = ACCENT_HEX_COLORS[accent];

    // ✅ 시스템 다크모드 상태 감지
    const colorScheme = useColorScheme();
    const isDark = colorScheme === 'dark';

    const [allDiaries, setAllDiaries] = useState<DiarySummary[]>([]);
    const [filteredDiaries, setFilteredDiaries] = useState<DiarySummary[]>([]);
    const [searchQuery, setSearchQuery] = useState("");
    const [viewMode, setViewMode] = useState<ViewModeType>("all");

    const [loading, setLoading] = useState(false);
    const [refreshing, setRefreshing] = useState(false);
    const [isFetchingMore, setIsFetchingMore] = useState(false);
    const [page, setPage] = useState(0);
    const [isEnd, setIsEnd] = useState(false);

    const fetchDiaries = useCallback(async (pageNum = 0, isRefresh = false) => {
        if (!user) return;

        if (isRefresh) {
            setLoading(true);
            setIsEnd(false);
        } else {
            setIsFetchingMore(true);
        }

        try {
            const response = await diaryApi.getDiaries("", pageNum, PAGE_SIZE, "diaryDate,desc");

            if (response?.result?.content) {
                const newContent = response.result.content;
                if (isRefresh) {
                    setAllDiaries(newContent);
                } else {
                    setAllDiaries(prev => [...prev, ...newContent]);
                }
                if (newContent.length < PAGE_SIZE || response.result.last) {
                    setIsEnd(true);
                }
            } else {
                if (isRefresh) setAllDiaries([]);
                setIsEnd(true);
            }
        } catch (error) {
            console.error("일기 목록 로드 실패", error);
            if (isRefresh) setAllDiaries([]);
        } finally {
            setLoading(false);
            setIsFetchingMore(false);
        }
    }, [user]);

    useFocusEffect(
        useCallback(() => {
            setPage(0);
            fetchDiaries(0, true);
        }, [fetchDiaries])
    );

    const onRefresh = useCallback(async () => {
        setRefreshing(true);
        setPage(0);
        await fetchDiaries(0, true);
        setRefreshing(false);
    }, [fetchDiaries]);

    const handleLoadMore = () => {
        if (!isEnd && !loading && !isFetchingMore && !refreshing) {
            const nextPage = page + 1;
            setPage(nextPage);
            fetchDiaries(nextPage, false);
        }
    };

    React.useEffect(() => {
        const filtered = allDiaries.filter(diary => {
            const d = diary as any;
            const lowerQuery = searchQuery.toLowerCase();
            const matchTitle = d.title?.toLowerCase().includes(lowerQuery);
            const matchSummary = (d.summary || d.content)?.toLowerCase().includes(lowerQuery);
            const matchTags = d.tags?.some((tag: any) => {
                const tagName = typeof tag === 'string' ? tag : tag.name;
                return tagName.toLowerCase().includes(lowerQuery);
            });
            const textMatch = !searchQuery.trim() || matchTitle || matchSummary || matchTags;
            const viewMatch = viewMode === 'photo'
                ? (!!d.imageUrl || (d.images && d.images.length > 0))
                : true;
            return textMatch && viewMatch;
        });
        setFilteredDiaries(filtered);
    }, [searchQuery, viewMode, allDiaries]);

    const handleDiaryClick = useCallback((diaryId: number) => {
        router.push({ pathname: '/diary-screen/viewer', params: { id: diaryId, origin: 'diary' } });
    }, [router]);

    const renderDiaryCard = useCallback(({ item }: { item: DiarySummary }) => {
        const d = item as any;
        const hasImage = !!d.imageUrl || (d.images && d.images.length > 0);
        const previewUrl = d.imageUrl || (d.images?.[0]?.url || d.images?.[0]);
        const dateStr = d.diaryDate || d.date || d.createdAt || d.createAt;
        const hasTags = d.tags && d.tags.length > 0;
        const hasContent = !!(d.summary || d.content);

        if (viewMode === 'photo') {
            return (
                <TouchableOpacity
                    activeOpacity={0.8}
                    onPress={() => handleDiaryClick(d.diaryId)}
                    style={{ width: '48%', aspectRatio: 1, marginBottom: scale(16), borderRadius: scale(20), overflow: 'hidden', backgroundColor: '#F1F5F9' }}
                >
                    <Image
                        source={{ uri: previewUrl }}
                        style={{ width: '100%', height: '100%' }}
                        contentFit="cover"
                        transition={300}
                    />
                    <View
                        style={{
                            position: 'absolute', bottom: 0, width: '100%',
                            paddingTop: scale(10), paddingBottom: scale(12), paddingHorizontal: scale(16),
                            backgroundColor: 'rgba(0,0,0,0.3)'
                        }}
                    >
                        <Text className="text-white font-black tracking-widest" style={{ fontSize: scale(11) }} allowFontScaling={false}>
                            {dateStr ? format(new Date(dateStr), "MM.dd", { locale: ko }) : ""}
                        </Text>
                    </View>
                </TouchableOpacity>
            );
        }

        return (
            <TouchableOpacity
                activeOpacity={0.8}
                onPress={() => handleDiaryClick(d.diaryId)}
                className="border-b border-slate-100 dark:border-slate-800/60"
                style={{ paddingHorizontal: scale(24), paddingVertical: scale(18) }}
            >
                <Text className="font-extrabold text-slate-500 dark:text-slate-400 uppercase tracking-widest" style={{ fontSize: scale(11), marginBottom: scale(6) }} allowFontScaling={false}>
                    {dateStr ? format(new Date(dateStr), "yyyy. MM. dd EEEE", { locale: ko }) : "날짜 없음"}
                </Text>

                <Text className="font-extrabold text-slate-900 dark:text-white tracking-tight" style={{ fontSize: scale(20), lineHeight: scale(28), marginBottom: hasContent || hasTags ? scale(8) : 0 }} allowFontScaling={false} numberOfLines={2}>
                    {d.title || "제목 없음"}
                </Text>

                {hasImage && previewUrl && (
                    <View className="w-full rounded-[1.5rem] overflow-hidden bg-slate-100 dark:bg-slate-800 border border-slate-100 dark:border-slate-800/60 shadow-sm" style={{ aspectRatio: 4 / 3, marginBottom: hasContent || hasTags ? scale(12) : 0 }}>
                        <Image source={{ uri: previewUrl }} style={{ width: '100%', height: '100%' }} contentFit="cover" transition={300} />
                    </View>
                )}

                {hasContent && (
                    <Text className="font-normal text-slate-600 dark:text-slate-300" style={{ fontSize: scale(15), lineHeight: scale(22), marginBottom: hasTags ? scale(14) : 0 }} allowFontScaling={false} numberOfLines={3}>
                        {d.summary || d.content}
                    </Text>
                )}

                {hasTags && (
                    <View className="flex-row flex-wrap" style={{ gap: scale(8) }}>
                        {d.tags.map((tag: any, idx: number) => {
                            const tagName = typeof tag === 'string' ? tag : tag.name;
                            return (
                                <View key={idx} className="bg-primary-50 dark:bg-primary-900/40 rounded-md border border-primary-100/50 dark:border-primary-800/50" style={{ paddingHorizontal: scale(10), paddingVertical: scale(6) }}>
                                    <Text className="text-primary-500 dark:text-primary-300 font-extrabold uppercase tracking-wide" style={{ fontSize: scale(11) }} allowFontScaling={false}>
                                        #{tagName}
                                    </Text>
                                </View>
                            );
                        })}
                    </View>
                )}
            </TouchableOpacity>
        );
    }, [viewMode, handleDiaryClick]);

    return (
        <SafeAreaView className="flex-1 bg-white dark:bg-slate-950" edges={['top']}>
            {/* 헤더 */}
            <View className="px-6 py-4 pb-2 bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl z-20">
                <RNText className="text-3xl font-extrabold text-slate-900 dark:text-white tracking-tight" style={{ fontFamily: customFontFamily }} allowFontScaling={false}>
                    Story
                </RNText>
            </View>

            {/* 검색 + 토글 */}
            <View className="bg-white/95 dark:bg-slate-950/95 backdrop-blur-xl z-10 border-b border-slate-100 dark:border-slate-800/60" style={{ paddingHorizontal: scale(24), paddingTop: scale(12), paddingBottom: scale(16) }}>
                <View className="flex-row items-center" style={{ gap: scale(12) }}>

                    {/* 검색창 */}
                    <View className="flex-1 flex-row items-center bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800" style={{ height: scale(46), paddingHorizontal: scale(16) }}>
                        <Ionicons name="search" size={scale(20)} color="#94A3B8" />
                        <TextInput
                            placeholder="검색어를 입력하세요..."
                            placeholderTextColor="#94A3B8"
                            className="flex-1 font-medium text-slate-900 dark:text-white py-0"
                            style={{ marginLeft: scale(12), fontSize: scale(15), paddingVertical: 0, marginVertical: 0, textAlignVertical: 'center', fontFamily: customFontFamily }}
                            value={searchQuery}
                            onChangeText={setSearchQuery}
                            clearButtonMode="while-editing"
                            returnKeyType="search"
                            allowFontScaling={false}
                        />
                    </View>

                    {/* ✅ 개선된 토글 버튼 (다크모드 지원 및 그림자 효과) */}
                    <View
                        className="flex-row bg-slate-50 dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800"
                        style={{ height: scale(46), padding: scale(4), gap: scale(2) }}
                    >
                        <TouchableOpacity
                            onPress={() => setViewMode('all')}
                            activeOpacity={0.8}
                            style={{
                                alignItems: 'center',
                                justifyContent: 'center',
                                borderRadius: scale(8),
                                paddingHorizontal: scale(12),
                                // 선택되었을 때, 다크모드면 slate-700 컬러, 라이트모드면 흰색 배경 적용
                                backgroundColor: viewMode === 'all' ? (isDark ? '#334155' : 'white') : 'transparent',
                                // 라이트 모드일 때만 살짝 입체감을 줌
                                shadowColor: viewMode === 'all' && !isDark ? "#000" : "transparent",
                                shadowOffset: { width: 0, height: 1 },
                                shadowOpacity: 0.05,
                                shadowRadius: 2,
                                elevation: viewMode === 'all' && !isDark ? 1 : 0,
                            }}
                        >
                            <Ionicons name="list" size={scale(20)} color={viewMode === 'all' ? accentHex : '#94A3B8'} />
                        </TouchableOpacity>

                        <TouchableOpacity
                            onPress={() => setViewMode('photo')}
                            activeOpacity={0.8}
                            style={{
                                alignItems: 'center',
                                justifyContent: 'center',
                                borderRadius: scale(8),
                                paddingHorizontal: scale(12),
                                backgroundColor: viewMode === 'photo' ? (isDark ? '#334155' : 'white') : 'transparent',
                                shadowColor: viewMode === 'photo' && !isDark ? "#000" : "transparent",
                                shadowOffset: { width: 0, height: 1 },
                                shadowOpacity: 0.05,
                                shadowRadius: 2,
                                elevation: viewMode === 'photo' && !isDark ? 1 : 0,
                            }}
                        >
                            <Ionicons name="grid" size={scale(20)} color={viewMode === 'photo' ? accentHex : '#94A3B8'} />
                        </TouchableOpacity>
                    </View>

                </View>
            </View>

            {/* 컨텐츠 */}
            <View className="flex-1 bg-white dark:bg-slate-950">
                {loading && !refreshing ? (
                    <View className="flex-1 items-center justify-center" style={{ paddingBottom: scale(80) }}>
                        <ActivityIndicator size="large" color={accentHex} />
                    </View>
                ) : filteredDiaries.length === 0 ? (
                    <ScrollView contentContainerStyle={{ flex: 1 }} refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={accentHex} />}>
                        <View className="flex-1 items-center justify-center opacity-60" style={{ paddingBottom: scale(80) }}>
                            <Ionicons name="document-text-outline" size={scale(48)} color="#94A3B8" style={{ marginBottom: scale(16) }} />
                            <Text className="text-slate-500 dark:text-slate-400 font-bold" style={{ fontSize: scale(14) }} allowFontScaling={false}>
                                {searchQuery ? "검색 결과가 없습니다." : viewMode === 'photo' ? "사진이 포함된 일기가 없습니다." : "작성된 일기가 없습니다."}
                            </Text>
                        </View>
                    </ScrollView>
                ) : (
                    <FlatList
                        key={viewMode === 'photo' ? 'grid-view' : 'list-view'}
                        data={filteredDiaries}
                        keyExtractor={(item) => item.diaryId.toString()}
                        renderItem={renderDiaryCard}
                        numColumns={viewMode === 'photo' ? 2 : 1}
                        columnWrapperStyle={viewMode === 'photo' ? { justifyContent: 'space-between', paddingHorizontal: scale(24) } : undefined}
                        contentContainerStyle={{ paddingBottom: scale(120), paddingTop: viewMode === 'photo' ? scale(24) : 0 }}
                        showsVerticalScrollIndicator={false}
                        keyboardDismissMode="on-drag"
                        refreshControl={<RefreshControl refreshing={refreshing} onRefresh={onRefresh} tintColor={accentHex} />}
                        onEndReached={handleLoadMore}
                        onEndReachedThreshold={0.5}
                        ListFooterComponent={
                            isFetchingMore ? (
                                <View style={{ paddingVertical: scale(24), alignItems: 'center' }}>
                                    <ActivityIndicator size="small" color={accentHex} />
                                </View>
                            ) : null
                        }
                    />
                )}
            </View>
        </SafeAreaView>
    );
}