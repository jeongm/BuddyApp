...
export default function PasswordChangeScreen() {
    ...
    const handleSavePassword = async () => {
        ...
        if (newPw !== confirmPw) {
            return Alert.alert("알림", "새 비밀번호와 비밀번호 확인 입력이 서로 다릅니다.");
        }
        if (currentPw === newPw) {
            return Alert.alert("알림", "새 비밀번호가 현재 비밀번호와 동일합니다.");
        }
    ...
    const isButtonDisabled = isPasswordSaving || !currentPasswordInput.trim() || !newPasswordInput.trim() || newPasswordInput !== confirmPasswordInput;
    ...
    return (
        <SafeAreaView className="flex-1 bg-white dark:bg-slate-950" edges={['top']}>
            ...
            <ScrollView className="flex-1" contentContainerStyle={{ paddingBottom: scale(100), paddingTop: scale(32) }} showsVerticalScrollIndicator={false}>
                ...
                <View style={{ paddingHorizontal: scale(20) }}>
                    <View className="bg-slate-50 dark:bg-slate-900 rounded-[24px] border border-slate-100 dark:border-slate-800/60 p-5" style={safeShadow}>
                        ...
                        {/* 1. 현재 비밀번호 */}
                        <View className="mb-5">
                            <Text className="font-bold text-slate-700 dark:text-slate-300 mb-2 ml-1" style={{ fontSize: scale(13) }} allowFontScaling={false}>현재 비밀번호</Text>
                            <View className="flex-row items-center bg-white dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700 rounded-xl px-4" style={{ height: scale(52) }}>
                                ...
                                />
                                <TouchableOpacity onPress={() => setSecureCurrent(!secureCurrent)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                                    <Ionicons name={secureCurrent ? "eye-off-outline" : "eye-outline"} size={scale(20)} color="#CBD5E1" />
                                </TouchableOpacity>
                            </View>
                        </View>

                        <View className="w-full h-[1px] bg-slate-200/60 dark:bg-slate-800/60 mb-5" />

                        {/* 2. 새 비밀번호 */}
                        <View className="mb-4">
                            <Text className="font-bold text-primary-600 dark:text-primary-400 mb-2 ml-1" style={{ fontSize: scale(13) }} allowFontScaling={false}>새 비밀번호</Text>
                            <View className="flex-row items-center bg-white dark:bg-slate-800 border border-slate-200/60 dark:border-slate-700 rounded-xl px-4" style={{ height: scale(52) }}>
                                <TextInput
                                    ...
                                    editable={!isPasswordSaving}
                                />
                                <TouchableOpacity onPress={() => setSecureNew(!secureNew)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
                        ...
                        <View>
                            <View className={`flex-row items-center bg-white dark:bg-slate-800 border ${getPasswordMatchInfo()?.props.children.includes('불일치') ? 'border-rose-400 dark:border-rose-500 bg-rose-50/30' : 'border-slate-200/60 dark:border-slate-700'} rounded-xl px-4`} style={{ height: scale(52) }}>
                                <TextInput
                                    ...
                                    editable={!isPasswordSaving}
                                />
                                <TouchableOpacity onPress={() => setSecureConfirm(!secureConfirm)} hitSlop={{ top: 10, bottom: 10, left: 10, right: 10 }}>
